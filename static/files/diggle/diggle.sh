...
echo -e "\e[1m\e[31m---------------------------------------------\e[0m"
echo -e "\e[32m  (\e[96m\e[1mGURU\e[0m\e[32m) checking dns for\e[34m" $1
echo -e "\e[1m\e[31m---------------------------------------------\e[0m"
while read p; do
        echo -e "\e[94m" $1 "\e[32m IN A       \e[35m" $p "\e[0m\e[95m " $(dig +short -x $p) "\e[0m"
done < <(dig +short @ns1.myukcloud.com in a $1)

while read p; do
	echo -e "\e[94m" $1 "\e[32m IN AAAA    \e[35m" $p "\e[0m\e[95m " $(dig +short -x $p) "\e[0m"
done < <(dig +short @ns1.myukcloud.com in aaaa $1)

while read p; do
        echo -e "\e[94m" $1 "\e[32m IN MX    \e[35m" $p "\e[0m"
done < <(dig +short @ns1.myukcloud.com in mx $1)

while read p; do
        echo -e "\e[94m" $1 "\e[32m IN TXT     \e[35m" $p "\e[0m"
done < <(dig +short @ns1.myukcloud.com in txt $1)

while read p; do
        echo -e "\e[94m" $1 "\e[32m IN SOA     \e[35m" $p "\e[0m"
done < <(dig +short @ns1.myukcloud.com in soa $1)
